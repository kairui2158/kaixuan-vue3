...
## 2026-08-29 设定层 UI P4 分类单行

1. 分类容器用 `nowrap` 表达单行意图，分类按钮设 `flex-shrink: 0`；窄宽度交给容器横向滚动，不把按钮压碎。
2. 响应式仍保留小屏整行降列的既定行为；本轮只改变桌面主布局，不扩大改动面。
3. 横向滚动验证必须注入收缩尺寸并读取 `scrollWidth/clientWidth/scrollLeft`，同时确认所有按钮的垂直位置一致。

## 2026-08-29 设定层 UI P5 四列小卡片

1. 设定卡片至少承载名称、两行摘要、绑定和信息操作；子按钮必须阻断卡片选中事件，避免一次点击产生两个语义。
2. 列宽用 `minmax(0, 1fr)` 防止长文本撑破栅格；列表设置内部最大高度和滚动，不把下方内容挤走。
3. 布局验证要同时读取栅格列数、卡宽、垂直坐标、滚动能力和实际点击后的状态；构建成功不能证明卡片按钮语义生效。

## 2026-08-29 设定层 UI P6 详情弹窗

1. 详情/编辑弹窗必须先持有当前设定快照，保存前只改弹窗内编辑态；关闭、遮罩点击和右上角关闭都不允许污染卡片、store 或项目 JSON。
2. 保存按钮是唯一回写边界，回写后立即核对卡片文本、store 数据和 `wa_project_*` storage 三者一致。
3. 删除操作沿用既有 `removeSetting`，弹窗关闭和数据移除要在同一次行为中验证；临时设定验证后必须恢复或删除，不得污染用户项目数据。

## 2026-08-29 设定层 UI P7 重启恢复

1. 持久化验证要先保存 `wa_project_*` 快照，再用临时设定走真实 UI；不要直接改项目 JSON，避免绕过 store 和保存链路。
2. 重启后的证据必须同时包含 storage 中的临时条目和真实卡片 DOM；二者一致才证明恢复到用户可见状态。
3. 验证后用 UI 删除临时条目，并将清理后的项目 JSON 与快照比对；只有完全一致才能声明未污染用户数据。

## 2026-08-29 设定层 UI P8 生成弹窗

1. 设定层生成反馈是居中模态，不是固定页面区块；遮罩全屏居中，日志区在模态内滚动，不挤压下方卡片。
2. 取消必须走 `pipelineStore.cancelGeneration()`，关闭只清空设定层日志，不得扩大成取消语义。
3. 验收必须同时提供居中坐标、日志滚动能力、真实取消和关闭后遮罩消失四类证据。

## 2026-08-29 设定层 UI P9 标题占位

1. 删除层级标题时必须同时确认字数联动提示和主要设定层容器仍然存在，不能只检查标题消失。
2. 重启后应用可能回到编辑器首屏；CDP 验证要先判断流水线是否已打开，再选择正确的入口和设定层步骤。
3. 标题删除的验收要结合真实 DOM 计数与截图，避免把“文本不可见”误判成“结构已清理”。

## 2026-08-29 设定层 UI P10 全量验收

1. 全量验收不能用分散的历史结果拼接；必须用同一个真实 Electron 会话复核布局、弹窗、store、项目 JSON 和取消行为。
2. Chromium 会把 `min(46vh, 420px)` 的 computed style 归一化为 `420px`；验收比较要覆盖归一化结果，不能把源码字符串当成运行时值。
