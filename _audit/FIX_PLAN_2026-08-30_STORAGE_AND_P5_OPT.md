...
## 后续增强清单（2026-08-31，API 对话显示，未排期）

背景：客户实测对话显示"主编体检报告"类长篇结构化输出时呈现质量差、阅读疲劳。对标 Codex/ChatGPT 级对话呈现。

- [ ] P0 文本选择与复制修复：全应用 body 设了 user-select:none，AI 对话气泡是 div 无恢复规则，导致内容无法鼠标选中、无法 Ctrl+C 复制（客户实测反馈）。修复：内容容器（对话气泡、共创气泡、流水线结果区、日志/预览区）显式 user-select:text，界面 chrome 保持不可选；回归验证 content-visibility 与跨屏选区复制。
- [ ] P1 表格渲染根治：AI 输出非法 Markdown 表格语法时退化为密集段落（实测截图确认无表格线）。需要双层修复：① 输出侧——SKILL/prompt 约束模型输出合法表格或 JSON 结构；② 渲染侧——容错解析（识别"伪表格"文本行并归一化为表格），`breaks: true` 导致的行堆叠需按内容类型调整段落节奏。
- [ ] P2 表格视觉升级：表头强调、单元格间距、行分隔/斑马纹、状态词（健康/警告/缺失/衰退）渲染为彩色状态徽章而非粗体字、列宽策略。
- [ ] P3 代码块增强：语法高亮（hljs/shiki，按需加载）+ 代码块头部 chrome（语言标签 + 独立复制按钮）。
- [ ] P4 流式渲染节流：每个增量 chunk 触发全文 marked 重解析，长回复低端机卡顿；改为 requestAnimationFrame/时间窗节流 + 增量渲染策略。
- [ ] P5 内容层级节奏：长报告的段落间距、标题层级、分区呼吸感对标主流 AI 对话框（Codex/ChatGPT）的间距尺度。
- 适用范围：以上各项覆盖全应用所有 AI 展示面（主页对话、技能绑定弹窗、大纲工作台、技能预览、生成流水线结果区等），完整盘点与执行计划见 `_audit/PROPOSAL_2026-08-31_CHAT_DISPLAY_UX.md`。
