...
### 差距1: 规则提取覆盖不足
现状: rules_raw.json只會65条规则，全部来自T12-T19(8层)。T01-T11和T20-T23共14层零覆盖。
根因: extract_rules.js有重复声明bug(两段完整代码拼接)，且Pattern 3/4/5未匹配到T01-T11的散落格式。
修复: P1删除旧文件重写，覆盖全5种格式，对14层缺失层逐层补提规则

### 差距2: 缺少逐组件级覆盖
现状: 计划只按手册层级映射到Vue3文件，没有逐个.vue组件的行为验证清单
修复: P3文件映射时为32个.vue组件各生成至少2条behavior规则(存在+交互)，确保每个组件都被检测

### 差距3: CSS回归检测缺失
现状: checkpoint显示P12 CSS验证35 PASS，但那是迁移阶段验证，不是差异检测。没有逐区块比对旧style.css(7489行)与新styles目录
修复: 新增P16 CSS回归检测步骤

### 差距4: Electron IPC通道级验证缺失
现状: 计划只在静态检测中检查contextIsolation/nodeIntegration，没有逐一验证18个IPC通道在Vue3中的实现
修复: 新增P17 IPC通道级验证步骤
...
`
Phase 1 规则提取(P1-P4) -- 串行，有依赖链
  P1: 重写extract_rules.js(删除bug, 支持5种pattern, 覆盖23层)
  P2: 规则分类标注(existence/value/behavior/state)
  P3: 规则到Vue3文件映射(32组件+21service+10store+8composable全覆盖)
  P4: 规则完整性校验 -> 冻结rules_final.json

Phase 2 静态检测(P5-P8) -- 可与Phase 3并行
  P5: 静态检测引擎(existence+value规则)
  P6: 静态结果分类汇总
  P7: 静态差异报告生成
  P8: 静态MISMATCH/MISSING修复判定

Phase 3 行为检测(P9-P12) -- 可与Phase 2并行
  P9: 行为检测框架(Playwright独立.js脚本)
  P10: 行为检测8组分组执行
  P11: 行为结果汇总
  P12: 行为差异报告

Phase 4 状态检测(P13-P15) -- 依赖Phase 3的Playwright环境
  P13: 状态检测引擎(store+localStorage一致性)
  P14: 状态结果分类
  P15: 状态差异报告

Phase 4.5 增强检测(P16-P17) -- v2.0新增
  P16: CSS回归检测(254变量+2088选择器逐区块比对)
  P17: Electron IPC通道级验证(18通道逐一)

Phase 5 汇总(P18-P20) -- 串行，依赖全部完成
  P18: 三引擎+CSS+IPC结果合并为差异矩阵
  P19: 修复优先级排序(P0-P4)
  P20: 最终差异报告DIFF_FINAL_REPORT_V2.md
...
| 编号 | 验证项 | 验证方法 | 通过标准 |
|------|--------|---------|----------|
| V1 | 规则完整性 | rules_final.json覆盖全部23层 | 规则数>=120, 23层全覆盖 |
| V2 | 静态检测覆盖 | static_results.json含全部existence+value规则 | 规则数=existence+value之和 |
| V3 | 行为检测覆盖 | behavior_results.json含全部behavior规则 | 8组全执行, 无SKIP |
| V4 | 状态检测覆盖 | state_results.json含全部state规则 | 规则数=state之和 |
| V5 | 组件覆盖 | 32个.vue组件各至少2条behavior规则检测 | 32/32组件覆盖 |
| V6 | CSS回归 | CSS对比报告覆盖254变量+2088选择器 | 变量全对齐, 选择器差异<5% |
| V7 | IPC通道 | 18个IPC通道逐一验证 | 18/18通道MATCH或ACCEPTABLE |
| V8 | 三引擎+增强合并 | diff_matrix.json规则数=rules_final.json | 数量一致 |
| V9 | 优先级排序 | 每个非MATCH项有P0-P4标签 | 无遗漏 |
| V10 | 证据完整 | P0/P1项有截图+DOM证据+文件路径 | 三者缺一未通过 |
| V11 | JSON合法性 | 全部JSON文件可JSON.parse | 无语法错误 |
| V12 | 报告可读性 | DIFF_FINAL_REPORT_V2.md含汇总+逐层+修复建议 | 结构完整 |
...
| 文件 | 产出阶段 | 用途 |
|------|---------|------|
| css_diff_report.md | P16 | CSS回归检测报告 |
| ipc_channel_report.md | P17 | IPC通道级验证报告 |
| diff_matrix_v2.json | P18 | 含CSS+IPC增强的差异矩阵 |
| fix_priority_v2.json | P19 | 增强版修复优先级清单 |
| DIFF_FINAL_REPORT_V2.md | P20 | v2.0最终差异报告 |
| verification_report_v2.md | P21 | v2.0交叉验证报告 |
...
1. Phase 2(P5-P8静态检测) 与 Phase 3(P9-P12行为检测) 可并行
   - 静态检测只读源码文件，不依赖运行时
   - 行为检测需要dev server运行，但不修改源码
   - 两者无数据依赖，可派2个子Agent并行

2. Phase 4.5的P16(CSS回归) 与 P17(IPC通道) 可并行
   - CSS只比对样式文件，IPC只检查electron目录
   - 两者文件范围不交叉

### 不可并行的Phase

1. Phase 1(P1-P4): 规则提取有严格依赖链
2. Phase 5(P18-P20): 合并依赖全部引擎完成
3. Phase 6(P21-P22): 验证依赖最终报告

### 子Agent调度策略

| Agent编号 | 任务 | 输入 | 输出 | 预计耗时 |
|----------|------|------|------|----------|
| Agent-A | Phase 2静态检测(P5-P8) | rules_final.json + Vue3源码 | static_results.json | 中等 |
| Agent-B | Phase 3行为检测(P9-P12) | rules_final.json + dev server | behavior_results.json + 截图 | 较长 |
| Agent-C | P16 CSS回归 | 旧style.css + 新styles/ | css_diff_report.md | 短 |
| Agent-D | P17 IPC验证 | 旧preload.js + 新electron/ | ipc_channel_report.md | 短 |

注意: Agent-B和Agent-C/D可与Agent-A并行，但Agent-B占用Playwright，Agent-C/D只读文件不冲突。
...
1. 对T01-T11和T20-T23共14层缺失层，P1脚本提取后必须人工(或子Agent辅助)逐层验证是否有规则遗漏
2. 每层至少3条规则(存在性+值+行为各至少1条)，不足3条的层需要从手册正文中补充提取
3. 32个.vue组件每个至少2条behavior规则(组件存在+核心交互)

### CSS回归检测细则

1. 逐变量比对: 旧style.css的254个CSS自定义属性(变量) vs 新styles/目录的变量定义
2. 逐选择器比对: 旧style.css的2088个选择器 vs 新styles/的选择器，标记MISSING的选择器
3. 逐区块比对: 将旧style.css按功能区块(导航/编辑器/聊天/设置/去AI味/章节树等)分段，与新CSS逐段对比
4. 主题变量比对: 暗色/亮色主题的变量值是否一致

### IPC通道验证细则

1. 列出旧preload.js暴露的全部18个IPC通道名
2. 在新electron/preload.ts中逐一查找是否存在
3. 对每个通道检查: 参数类型是否一致、返回值类型是否一致、错误处理是否一致
4. 特别检查: contextIsolation=true, nodeIntegration=false(安全边界)


---
*计划版本: v2.0 | 生成时间: 2026-08-11 | 生成者: Codex (GPT-5) | 增强内容: 补全14层规则+CSS回归+IPC通道级验证+组件覆盖*
