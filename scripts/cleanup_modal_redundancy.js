...
const patterns = /^(\.modal|\.modal-|#modal|\.panel|\.panel-|\.overlay-panel|\.card|\.card-|\.item-card|\.setting-card|\.skill-card|\.agent-card|\.card-grid|^html|^body|^main\b|#app-|#chapter-tree|#editor-panel|#chat-panel|#statusbar|#outline-workspace|#settings-collection|#pipeline-panel|#memory-panel|\.ow-hidden|\.sc-hidden|\.pl-hidden|\.mem-hidden|#inline-menu|#loading-indicator|\.overlay-panel)/;
...
for (const b of blocks) {
  ...
  const hasMatch = parts.some(p => patterns.test(p));
  if (!hasMatch) continue;
  const allMatch = parts.every(p => patterns.test(p) || p.length === 0);
  let impCount = 0;
