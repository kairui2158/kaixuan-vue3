    ...
    <section class="help-modal" role="dialog" aria-modal="true" aria-label="应用教学指南">
      ...
      <div class="help-layout">
        ...
        <article class="help-content">
          ...
          <template v-else-if="activeId === 'ai-naming'">
            <h3>AI 命名</h3>
            <p>AI 命名是一个独立的命名工作台：点击主界面的 AI 命名按钮即可打开，为角色、地点、势力或其他对象批量起名。</p>
            <ul>
              <li>先选择命名类别，也支持输入自定义类别，例如阵法、丹药、妖兽。</li>
              <li>填写背景与上下文、命名风格、数量和字数，角色类别还可以补充性别与种族。</li>
              <li>生成时会结合当前章节正文、项目大纲和已存在的名称，尽量避免重名。</li>
              <li>满意的名字可以收藏或复制；收藏和历史保存在当前项目里，切换项目不会串用。</li>
            </ul>
          </template>

          <template v-else-if="activeId === 'plugins'">
            <h3>插件市场</h3>
            <p>插件市场用于搜索和安装社区分享的智能体与技能，入口在设置页。</p>
            <ul>
              <li>输入关键词搜索 GitHub 仓库，可按智能体、技能分类筛选，并按星标或更新时间排序。</li>
              <li>配置 GitHub Token 后可以提高搜索配额；Token 只保存在本机。</li>
              <li>点击安装后，智能体和技能会导入到设置页对应列表，可在技能与智能体管理中查看或编辑。</li>
              <li>安装第三方内容后，建议先在小范围任务里试运行，确认指令符合预期再投入使用。</li>
            </ul>
          </template>

          <template v-else-if="activeId === 'appearance'">
            <h3>外观设置</h3>
            <p>外观设置位于设置页，用来调整界面字体、编辑器字体、字体大小和主题配色。</p>
            <ul>
              <li>界面字体影响菜单、面板和弹窗；编辑器字体只影响正文编辑区。</li>
              <li>字体大小与主题修改后点击保存即可生效，并会随项目环境持久保存。</li>
              <li>如果界面文字显示异常，可以先恢复默认字体，再逐步调回偏好的组合。</li>
            </ul>
          </template>
