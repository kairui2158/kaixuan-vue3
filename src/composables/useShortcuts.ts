...
export function useShortcuts(cb: ShortcutCallbacks) {
  function _handler(e: KeyboardEvent) {
    ...
    if (ctrl && !e.shiftKey && !e.altKey) {
      ...
      if (e.key === 'f' || e.key === 'F') { e.preventDefault(); cb.onFind?.(); return }
      if (e.key === 'k' || e.key === 'K') { e.preventDefault(); cb.onClearChat?.(); return }
    }

    // Ctrl+Shift+N: new project, Ctrl+Shift+P: project manager
    if (ctrl && e.shiftKey && !e.altKey) {
      if (e.key === 'n' || e.key === 'N') { e.preventDefault(); cb.onNewProject?.(); return }
      if (e.key === 'p' || e.key === 'P') { e.preventDefault(); cb.onProjectManager?.(); return }
    }
