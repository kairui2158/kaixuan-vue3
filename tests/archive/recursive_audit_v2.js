...
(async () => {
  ...
  // LAYER 2: Pipeline
  await hardReset();
  await page.evaluate(() => { const b = document.querySelector("#btn-pipeline"); if (b) b.click(); });
  await waitForStable(12000);
  ...
  await hardReset();
  await page.evaluate(() => { const b = document.querySelector("#btn-settings-collection"); if (b) b.click(); });
  await waitForStable(12000);
  ...
  // LAYER 2: Memory
  await hardReset();
  await page.evaluate(() => { const b = document.querySelector("#btn-memory"); if (b) b.click(); });
  await waitForStable(12000);
